...
scan_architectures() {
    ...
    vr_log::info "VERIFY" "[XCF] Scanning architectures for $module_name..."
    ...
    if [[ -d "$xcframework_path/ios-arm64" ]] || [[ -d "$xcframework_path/ios-arm64_x86_64-simulator" ]]; then
        has_arm64=1
        vr_log::info "VERIFY" "[XCF] Found arm64 architecture"
    else
        vr_log::warn "VERIFY" "[XCF] Missing arm64 architecture"
        warnings=$((warnings + 1))
    fi
    
    if [[ -d "$xcframework_path/ios-arm64_x86_64-simulator" ]] || [[ -d "$xcframework_path/ios-arm64-simulator" ]]; then
        has_arm64_simulator=1
        vr_log::info "VERIFY" "[XCF] Found arm64-simulator architecture"
    else
        vr_log::warn "VERIFY" "[XCF] Missing arm64-simulator architecture (simulator support may be limited)"
        warnings=$((warnings + 1))
    fi
    
    if [[ -d "$xcframework_path/ios-arm64_x86_64-simulator" ]] || [[ -d "$xcframework_path/ios-x86_64-simulator" ]]; then
        has_x86_64=1
        vr_log::info "VERIFY" "[XCF] Found x86_64 architecture"
    else
        vr_log::warn "VERIFY" "[XCF] Missing x86_64 architecture (Intel Mac simulator support may be limited)"
        warnings=$((warnings + 1))
    fi
    ...
    if [[ -n "$binary_path" ]] && [[ -f "$binary_path" ]] && command -v lipo >/dev/null 2>&1; then
        ...
        if [[ -n "$lipo_info" ]]; then
            vr_log::info "VERIFY" "[XCF] Binary architectures: $lipo_info"
        fi
    fi

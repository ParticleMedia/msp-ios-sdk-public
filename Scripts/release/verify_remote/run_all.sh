...
run_spm_verification() {
    if [[ "$REMOTE_VERIFY_ENABLED" != "1" ]] || [[ "$REMOTE_VERIFY_SPM" != "1" ]]; then
        vr_log_info "[VR] SPM remote verification skipped (disabled)"
        return 0
    fi
    
    # Check if required environment variables are set
    if [[ -z "${MSP_VERIFY_SPM_URL:-}" ]] || [[ -z "${MSP_VERIFY_SPM_VERSION:-}" ]]; then
        vr_log_warn "[VR] SPM remote verification skipped (missing MSP_VERIFY_SPM_URL / MSP_VERIFY_SPM_VERSION)"
        return 0
    fi
    
    vr_log_info "[VR] Running SPM remote verification..."
    REMOTE_SPM_EXECUTED=1
    
    if "$SCRIPT_DIR/spm/verify_spm_release.sh" 2>&1; then
        REMOTE_SPM_SUCCESS=1
        vr_log_info "[VR] SPM remote verification succeeded"
        return 0
    else
        REMOTE_SPM_SUCCESS=0
        vr_log_error "[VR] SPM remote verification failed"
...
run_pods_verification() {
    if [[ "$REMOTE_VERIFY_ENABLED" != "1" ]] || [[ "$REMOTE_VERIFY_PODS" != "1" ]]; then
        vr_log_info "[VR] Pods remote verification skipped (disabled)"
        return 0
    fi
    
    # Check if required environment variables are set
    if [[ -z "${MSP_VERIFY_PODS_URL:-}" ]] || [[ -z "${MSP_VERIFY_PODS_VERSION:-}" ]]; then
        vr_log_warn "[VR] Pods remote verification skipped (missing MSP_VERIFY_PODS_URL / MSP_VERIFY_PODS_VERSION)"
        return 0
    fi
    
    vr_log_info "[VR] Running Pods remote verification..."
    REMOTE_PODS_EXECUTED=1
    
    if "$SCRIPT_DIR/cocoapods/verify_pods_release.sh" 2>&1; then
        REMOTE_PODS_SUCCESS=1
        vr_log_info "[VR] Pods remote verification succeeded"
        return 0
    else
        REMOTE_PODS_SUCCESS=0
        vr_log_error "[VR] Pods remote verification failed"
        return 0  # Soft-fail: return 0 to not break release
    fi
}
...
run_all_remote_verification() {
    # Run SPM verification
    run_spm_verification || true
    
    # Run CocoaPods verification

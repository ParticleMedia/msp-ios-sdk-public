...
verify_spm_release() {
    ...
    if ! command -v swift >/dev/null 2>&1; then
        vr_log::warn "SPM" "swift command not found, skipping SPM verification"
        return 0
    fi
    ...
    if [[ -z "$remote_url" ]] || [[ -z "$remote_version" ]]; then
        vr_log::warn "SPM" "SPM remote verification skipped (missing MSP_VERIFY_SPM_URL / MSP_VERIFY_SPM_VERSION)"
        return 0
    fi
    
    local SANDBOX_DIR
    SANDBOX_DIR="$(vr_create_sandbox "spm")" || {
        vr_log::error "SPM" "Failed to create sandbox"
        return 1
    }
    ...
    vr_log::info "SPM" "Starting SPM remote verification (URL=$remote_url, version=$remote_version)"
    
    # Step 1: Prepare DemoApp
    if ! "$SCRIPT_DIR/prepare_demoapp.sh" "$SANDBOX_DIR"; then
        vr_log::error "SPM" "Failed to prepare DemoApp"
        return 1
    fi
    
    # Step 2: Parse remote Package.swift
    vr_log::info "SPM" "[SPM] Parsing remote Package.swift..."
    # Source the script to get the function, then call it
    source "$SCRIPT_DIR/parse_remote_package.sh"
    if ! parse_remote_package "$SANDBOX_DIR" "$remote_url"; then
        vr_log::warn "SPM" "[SPM] Remote package analysis failed — skipping SPM verify"
        return 0
    fi
    
    if [[ -z "${REMOTE_PACKAGE_NAME:-}" ]] || [[ -z "${REMOTE_PRODUCT_NAME:-}" ]]; then
        vr_log::warn "SPM" "[SPM] Remote package analysis failed (missing package/product name) — skipping SPM verify"
        return 0
    fi
    ...
    if ! "$SCRIPT_DIR/patch_package.swift.sh" "$SANDBOX_DIR" "$remote_url" "$remote_version"; then
        vr_log::error "SPM" "Failed to patch Package.swift"
        return 1
    fi
    
    # Step 4: Build DemoApp
    if ! "$SCRIPT_DIR/build_demoapp.sh" "$SANDBOX_DIR"; then
        vr_log::error "SPM" "Failed to build DemoApp"
        return 1
    fi
    
    vr_log::info "SPM" "SPM remote verification (URL=$remote_url, version=$remote_version) completed successfully."

...
RUBY_SCRIPT
    ...
    if [[ "$publish_exit_code" == "0" ]]; then
        ...
        # Auto-retry on CocoaPods server errors (HTTP 500 / CDN issues)
        if grep -q "internal server error" "$log_file" || grep -q "server error occurred" "$log_file"; then
            ...
            while [[ $server_retry -le $max_server_retries ]]; do
                ...
                # If no longer a server error, break out and fall through to real failure
                if ! grep -q "internal server error" "$log_file" && ! grep -q "server error occurred" "$log_file"; then
                    log_warn "Error is no longer a server error, stopping retries"
                    break
                fi

#!/usr/bin/env python3
"""
Update getSDKVersion() string literal in Swift adapter sources.

Exit codes:
  0: Success (updated or already matches)
  1: Check mode only - version differs
  2: Function not found
  3: String literal not found inside function
  4: IO error
"""
...
def main() -> int:
    ...
    for path in swift_files:
        status, _changed, mismatch = _update_file(path, args.function, args.version, args.check)
        if status == "io_error":
            return 4

...
# Create GitHub release and zip file for a pod (for skipped pods)
create_github_release_for_skipped_pod() {
    local pod_name="$1"
    local version="$2"
    
    log_step "Creating GitHub release and zip file for skipped pod: $pod_name $version"
    
    # Check if GitHub release already exists
    if gh release view "$version" --repo ParticleMedia/msp-ios-sdk-public >/dev/null 2>&1; then
        log_info "GitHub release $version already exists, checking for $pod_name zip file"
        
        # Check if the specific pod zip file exists in the release
        if gh release view "$version" --repo ParticleMedia/msp-ios-sdk-public --json assets --jq '.assets[].name' | grep -q "${pod_name}-${version}.zip"; then
            log_info "Zip file ${pod_name}-${version}.zip already exists in release $version"
            return 0
        fi
    fi
    
    # Create zip file
    local zip_file="${pod_name}-${version}.zip"
    if [[ -d "$pod_name" ]]; then
        log_step "Creating zip file: $zip_file"
        if zip -r "$zip_file" "$pod_name/" >/dev/null 2>&1; then
            log_success "Created zip file: $zip_file"
        else
            log_error "Failed to create zip file: $zip_file"
            return 1
        fi
    else
        log_error "Directory $pod_name not found, cannot create zip file"
        return 1
    fi
    
    # Create or update GitHub release
    if gh release view "$version" --repo ParticleMedia/msp-ios-sdk-public >/dev/null 2>&1; then
        # Release exists, upload the zip file
        log_step "Uploading $zip_file to existing release $version"
        if gh release upload "$version" "$zip_file" --repo ParticleMedia/msp-ios-sdk-public; then
            log_success "Uploaded $zip_file to release $version"
            rm -f "$zip_file"  # Clean up local zip file
            return 0
        else
            log_error "Failed to upload $zip_file to release $version"
            rm -f "$zip_file"
            return 1
        fi
    else
        # Create new release
        log_step "Creating new GitHub release $version with $zip_file"
        if gh release create "$version" --repo ParticleMedia/msp-ios-sdk-public --title "$version" --notes "Release $version" "$zip_file"; then
            log_success "Created GitHub release $version with $zip_file"
            rm -f "$zip_file"  # Clean up local zip file
            return 0
        else
...
release_single_pod() {
    ...
    if check_pod_version_published "$pod_name" "$version"; then
        ...
        # Still create GitHub release and zip file for dependencies
        if [[ "$pod_name" == "MSPSharedLibraries" || "$pod_name" == "PrebidAdapter" ]]; then
            log_info "Creating GitHub release and zip file for $pod_name (needed for MSPCore dependencies)"
            if ! create_github_release_for_skipped_pod "$pod_name" "$version"; then
                log_error "Failed to create GitHub release for skipped pod $pod_name"
                return 1
            fi
        fi

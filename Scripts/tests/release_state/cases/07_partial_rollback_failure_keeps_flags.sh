...
output=$(cat "${repo_root}/rollback_output.log" || echo "")
...
mock_log_contains "$MOCK_LOG" "git tag -d v0.0.1" "Tag deletion should occur"

# Verify branch deletion was attempted
mock_log_contains "$MOCK_LOG" "git push origin --delete release/0.0.1" "Branch deletion should be attempted"

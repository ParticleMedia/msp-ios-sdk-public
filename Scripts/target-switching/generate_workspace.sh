...
YAML
    ...
    # Both pods-dev and pods-release need the [CP] Copy XCFrameworks prebuild phase.
    # CocoaPods would normally wire xcframework staging via PBXTargetDependency on each
    # public xcframework Pod target (e.g. AppLovinMediationGoogleAdapter, FBAudienceNetwork).
    # In this project the MSPDemoApp pbxproj is regenerated by XcodeGen from this template
    # AFTER pod install runs, which strips those PBXTargetDependency entries — leaving the
    # host with `-framework X` link flags but no build phase to stage X.framework into
    # PODS_XCFRAMEWORKS_BUILD_DIR. This script-driven phase finds every `*-xcframeworks.sh`
    # under Pods/Target Support Files/ and runs it, which makes the staging deterministic
    # regardless of pod-target dependency wiring.
    #
    # Historical note: pods-dev mode used to skip this phase on the assumption that all pods
    # were source-based (compiled directly into the Pods static library, no staging needed).
    # PR #742 broke that assumption by adding AppLovinMediationGoogleAdapter — a public
    # xcframework-only pod — to the pods-dev Podfile. The phase is harmless in source-only
    # builds (the find returns no scripts) and required when any xcframework pod is present.
    echo "${TARGET_MODE}: Adding [CP] Copy XCFrameworks and [CP] Embed Pods Frameworks phases" >&2
    cat <<'YAML' >> "$MODE_TARGETS_FILE"
    prebuildScripts:

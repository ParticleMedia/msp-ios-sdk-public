...
tags:
  ...
  - tee
triggers:
  - "pod trunk push succeeds but version not published"
  - "shell pipeline exit code incorrect"
  - "PIPESTATUS tee exit code"
  - "CI shows success but pod not released"
summary: "Shell pipeline with tee masks pod trunk push failure; use PIPESTATUS[0] to capture real exit code"
version: "1.0"
status: active
created: "2026-01-29"
updated: "2026-02-22"
...
| Playbook | 关系 | 何时参考 |
|----------|------|----------|
| [ctx-sources-004](../../sources/tech/ctx-sources-004-script-best-practices.md) | **上游 — 脚本最佳实践** | Shell 脚本的 `set -euo pipefail`、错误处理原则；本文是其 PIPESTATUS 具体案例 |
| [ctx-ci-004](../../ci/experience/ctx-ci-004-preinstall-source-dependency.md) | **同类 — CI 脚本陷阱** | 另一种 CI 脚本中不易发现的执行时序问题（chicken-egg vs exit code） |

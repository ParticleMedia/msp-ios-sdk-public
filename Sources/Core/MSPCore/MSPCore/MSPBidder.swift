...
public class MSPBidder: MSPiOSCore.Bidder {
    ...
    public override func requestBid(
        ...
        adRequest.customParams["ppid"] = MSP.shared.ppid
        for (key, value) in MSPDevice.shared.getDeviceSignalsDictionary() {
            adRequest.customParams[key] = value
        }
        prebidBidLoader.loadBid(
            placementId: bidderPlacementId, adParams: adRequest.customParams, bidListener: self, adRequest: adRequest)
    }
            ...
            let auctionBidListener = auctionBidListener
        {
            if let adNetworkAdapter = MSP.shared.adNetworkAdapterProvider.getAdNetworkAdapter(adNetwork: adNetwork) {
                ...
                DispatchQueue.main.async {
                    ...
                    adNetworkAdapter.loadAdCreative(
                        bidResponse: bidResponse, auctionBidListener: auctionBidListener, adListener: adListener,
                        context: self, adRequest: adRequest, bidderPlacementId: self.bidderPlacementId,
                        bidderFormat: nil, params: self.params)
                }
            } else {

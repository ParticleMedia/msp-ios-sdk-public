...
@objc public class UnityAdapter: NSObject, AdNetworkAdapter {
    ...
    public func handleAdLoaded(ad: MSPAd, auctionBidListener: AuctionBidListener, bidderPlacementId: String) {
        ...
        let auctionBid = AuctionBid(
            bidderName: "unity", bidderPlacementId: bidderPlacementId, ecpm: ad.adInfo["price"] as? Double ?? 0.0)
        auctionBid.ad = ad
        ...
        DispatchQueue.main.async {
                ...
                let auctionBidListener = self.auctionBidListener
            {
                ...
                self.handleAdLoaded(
                    ...
                    bidderPlacementId: self.bidderPlacementId ?? "unity_placement_id")
    ...
    public func didLoad(_ nativeAd: LevelPlayNativeAd, with adInfo: ISAdInfo) {
        DispatchQueue.main.async {
            ...
            if let auctionBidListener = self.auctionBidListener {
                let unityNativeAd = UnityNativeAd(
                    ...
                    let auctionBidListener = self.auctionBidListener
                {
                    ...
                    self.handleAdLoaded(
                        ad: unityNativeAd, auctionBidListener: auctionBidListener,
                        bidderPlacementId: self.bidderPlacementId ?? adRequest.placementId)
                    self.adMetricReporter?.logAdResult(
                        placementId: adRequest.placementId, ad: unityNativeAd, fill: true, isFromCache: false)
                }
            }
        }
    }

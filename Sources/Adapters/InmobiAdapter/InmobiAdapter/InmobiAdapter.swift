        ...
        DispatchQueue.main.async {
                ...
                let auctionBidListener = self.auctionBidListener
            {
                ...
                self.handleAdLoaded(
                    ...
                    bidderPlacementId: self.bidderPlacementId ?? "inmobi_placement_id")
            }
        }
    }
        ...
        DispatchQueue.main.async {
                ...
                let auctionBidListener = self.auctionBidListener
            {
                ...
                self.handleAdLoaded(
                    ...
                    bidderPlacementId: self.bidderPlacementId ?? "inmobi")
            }
        }
    }

    public func interstitial(
        _ interstitial: InMobiSDK.IMInterstitial, didReceiveWithMetaInfo metaInfo: InMobiSDK.IMAdMetaInfo
    ) {
        DispatchQueue.main.async {
    ...
    public func nativeDidFinishLoading(_ native: IMNative) {
        DispatchQueue.main.async {
            ...
            if let auctionBidListener = self.auctionBidListener {
                ...
                let inmobiNativeAd = InmobiNativeAd(
                    ...
                    let auctionBidListener = self.auctionBidListener
                {
                    ...
                    self.handleAdLoaded(
                        ad: inmobiNativeAd, auctionBidListener: auctionBidListener,
                        bidderPlacementId: self.bidderPlacementId ?? adRequest.placementId)
                    self.adMetricReporter?.logAdResult(
                        placementId: adRequest.placementId, ad: inmobiNativeAd, fill: true, isFromCache: false)
                }
            }
        }
    }
